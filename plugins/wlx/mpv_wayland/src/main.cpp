...
HWND DCPCALL ListLoadW(HWND ParentWin, WCHAR *FileToLoad, int ShowFlags) {
  ...
  // QString::fromWCharArray works with wchar_t*, but WCHAR is uint16_t in
  // common.h
  QString fileName =
      QString::fromUtf16(reinterpret_cast<const char16_t *>(FileToLoad));
...
int DCPCALL ListLoadNextW(HWND ParentWin, HWND PluginWin, WCHAR *FileToLoad,
                          int ShowFlags) {
  (void)ParentWin;
  (void)ShowFlags;
...
void DCPCALL ListSetDefaultParams(ListDefaultParamStruct *dps) { (void)dps; }
