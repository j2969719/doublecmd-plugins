...
static void query_clicked_cb(GtkButton *button, CustomData *data)
{
	...
	GtkWindow *parent = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(data->list)));
	GtkWidget *dialog = gtk_dialog_new_with_buttons(_("Query"), parent, flags,
...
HWND DCPCALL ListLoad(HWND ParentWin, char* FileToLoad, int ShowFlags)
{
	...
	gtk_box_pack_end(GTK_BOX(gFix), scroll, TRUE, TRUE, 10);
