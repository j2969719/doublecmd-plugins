...
if plug then
  for file, info in wcx.list_archive(plug, "zip.zip", true) do
    print(info.mode, info.filetime, info.pk_size, info.unpk_size, file)
    --print(info.mode, os.date("%Y-%m-%d %T", tonumber(info.filetime)), info.pk_size, info.unpk_size, file)
