...
- `make_all.sh`: allows to compile all possible plugins (don't forget to install all required packages for the necessary (or all) plugins, also see the note about the `bit7z` library below);<br>
- `make_all_clean.sh`: cleanup after compilation, i.e. `make clean`, but for all plugins;<br>
- `pack_to_tarballs.sh`: pack all plugins into separate archives (`make dist`);<br>
